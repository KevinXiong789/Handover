...
public:
    ...
    {
        joints_publisher_ = this->create_publisher<std_msgs::msg::Float32MultiArray>("/Nuitrack/cameraR/joints_position", 10);
        hand_publisher_ = this->create_publisher<std_msgs::msg::Float32MultiArray>("/Nuitrack/right_hand1", 10);
        // Initialize publishers for depth image and point cloud
        depth_cloud_pub_ = this->create_publisher<sensor_msgs::msg::PointCloud2>("/Nuitrack/cameraR/depth_cloud", 10);
        marker_array_publisher_ = this->create_publisher<visualization_msgs::msg::MarkerArray>("/Nuitrack/cameraR/joint_markers", 10);
 
        Nuitrack::init("");
        auto devices = Nuitrack::getDeviceList();
        auto selectedDevice = devices[1]; // 0 is top, 1 is right, 2 is left
...
private:
    void onSkeletonUpdate(SkeletonData::Ptr skeletonData)
    {
        ...
        // Translation
        transform(0, 3) = -0.4614380; 
        transform(1, 3) = 0.2186506;
        transform(2, 3) = 0.2227520; 

        // Rotation (using roll, pitch, yaw)
        double yaw = 0.75865274;
        double pitch = -0.00171671;
        double roll = -0.01183805;
    ...
    void publishJointMarkers(const std::vector<Skeleton>& skeletons) {
        ...
        for (const Skeleton& skeleton : skeletons) {
            for (const Joint& joint : skeleton.joints) {
                ...
                marker.header.frame_id = "cameraR_link"; 
