...
public:
    ...
    {
        joints_publisher_ = this->create_publisher<std_msgs::msg::Float32MultiArray>("/Nuitrack/cameraL/joints_position", 10);
        ...
        depth_cloud_pub_ = this->create_publisher<sensor_msgs::msg::PointCloud2>("/Nuitrack/cameraL/depth_cloud", 10);
        marker_array_publisher_ = this->create_publisher<visualization_msgs::msg::MarkerArray>("/Nuitrack/cameraL/joint_markers", 10);
 
        Nuitrack::init("");
        auto devices = Nuitrack::getDeviceList();
        auto selectedDevice = devices[2]; // 0 is top, 1 is right, 2 is left
...
private:
    void onSkeletonUpdate(SkeletonData::Ptr skeletonData)
    {
        ...
        // Translation
        transform(0, 3) = 0.4982635; 
        transform(1, 3) = 0.2166730;
        transform(2, 3) = 0.2317470; 

        // Rotation (using roll, pitch, yaw)
        double yaw = 2.34878000;
        double pitch = 0.00054563;
        double roll = 0.00011913;
    ...
    void publishJointMarkers(const std::vector<Skeleton>& skeletons) {
        ...
        for (const Skeleton& skeleton : skeletons) {
            for (const Joint& joint : skeleton.joints) {
                ...
                marker.header.frame_id = "cameraL_link"; 
    ...
    void onNewDepthFrame(DepthFrame::Ptr frame)
    {
        ...
        cloud_msg.header.frame_id = "cameraL_link";
