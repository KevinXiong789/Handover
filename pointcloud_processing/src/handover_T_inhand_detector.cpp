...
public:
  SubscriberNode() : Node("Handover_detector")
  {
    ...
    pcl_subscriber_ = create_subscription<sensor_msgs::msg::PointCloud2>(
      "/raw_final_pointscloud", 10, std::bind(&SubscriberNode::processPointCloud, this, std::placeholders::_1));

    // Subscribe to the Float32MultiArray topic
    float_array_subscriber_ = create_subscription<std_msgs::msg::Float32MultiArray>(
      "/Openpose/transformed_handsPosition", 10, std::bind(&SubscriberNode::processFloatArray, this, std::placeholders::_1));

    // Create a publisher to publish the processed point cloud
    pcl_publisher = create_publisher<sensor_msgs::msg::PointCloud2>("/hand_pointscloud", 1);
...
private:
  ...
  bool is_in_the_cell(){
    if (right_hand_x >= -0.7 && right_hand_x <= 0.7 &&
        right_hand_y >= 0.0 && right_hand_y <= 1.5 &&
        right_hand_z >= 0.0 && right_hand_z <= 1.0){
          return true;
        }
    return false;
  }
  ...
  void processPointCloud(const sensor_msgs::msg::PointCloud2::SharedPtr msg)
  {
    ...
    if (!cropped_cloud->empty()){
      ...
      vg_filter.filter(*processed_cloud);
    ...
    /*
    // ************testing***************************************************************************************
    float distance = distanceComputing(tool_max_point, wrist_point);

    // Write distance to a txt file
    std::ofstream outFile;
    outFile.open("output.txt", std::ios::app);  // Open the file in append mode
    outFile << distance << std::endl;
    outFile.close();
    RCLCPP_INFO(get_logger(), "Distance between max_point and wrist_point: %.4f", distance);
    // ***********************************************************************************************************
    */
    ...
    if (!isnan(tool_max_point[0]) && distanceComputing(tool_max_point, wrist_point)> distance_threshold){
      ...
      //for yaw
      double Yaw_angle = std::acos(tool_vector.dot(x_vector) / tool_vector.norm());
      float yaw_angle = Yaw_angle - M_PI/2;
      ...
      //float yaw_command_deg = pcl::rad2deg(Yaw_angle)-90;
      // just use yaw
      //RCLCPP_INFO(get_logger(), "roll: %.4f, pitch: %.4f, yaw: %.4f \n", roll_command_deg, pitch_command_deg, yaw_command_deg);
      //RCLCPP_INFO(get_logger(), "yaw: %.4f \n", Yaw_angle);
      ...
      tool_grasping_position_msg.data = {toolgrasp_centroid(0),toolgrasp_centroid(1),toolgrasp_centroid(2),yaw_angle};
  ...
  void processFloatArray(const std_msgs::msg::Float32MultiArray::SharedPtr msg)
  {
    ...
    right_hand_x = msg->data[3];
    right_hand_y = msg->data[4];
    right_hand_z = msg->data[5];
    right_elbow_x = msg->data[0];
    right_elbow_y = msg->data[1];
    right_elbow_z = msg->data[2];
